...
$.widget("mapQuery.mqPopup", {
    ...
    _onFeatureselected: function(evt, data) {
        ...
        var contents = self.options.contents.call(this, data);

        // save position so that the popup can be moved with the feature
        self.lonLat = $.MapQuery.getFeaturePosition(data.olFeature);
        ...
        }).draggable({handle:'.ui-dialog-titlebar'}).resizable();
        ...
        element.html($.tmpl('mqPopup', {
            ...
            element.hide();
            self.lonLat = null;
            layer.unselectFeature(data.olFeature);
        });
